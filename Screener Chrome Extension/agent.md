...
1. Bump the version in `manifest.json` (and keep `extension-code/manifest-firefox.json` in sync).
...
## Firefox (AMO) Port
- Firefox-only files live in `firefox/` (currently just `manifest.json`). `extension-code/` stays purely Chrome.
- Firefox has no `sidePanel` API: the Firefox build uses `sidebar_action` with the same `sidepanel.html`. Each engine ignores the other's key.
- Firefox build uses an event-page background (`background.scripts`), not a service worker. Sources are shared from `extension-code/`; only `manifest.json` differs.
- `background.js` must never touch `chrome.sidePanel` unguarded (it is `undefined` on Firefox and throws at load). Notification `iconUrl` must be a packaged icon, never `chrome://favicon/...`. Both guards are no-ops on Chrome.
- Build with `powershell -ExecutionPolicy Bypass -File dev-tools/build-firefox.ps1` → `Screener-Extension-Firefox-vX.Y.Z.zip`. Validate with `npx -y web-ext lint` (0 errors expected; `sidePanel` + `innerHTML` warnings are benign).
